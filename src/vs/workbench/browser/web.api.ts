...
export interface IEduCompilerServiceConfiguration {
    /** Base URL for the Universal Compiler API endpoint. */
    readonly endpoint: string;

    /** Optional HTTP headers that should be sent with each request. */
    readonly headers?: Record<string, string>;

    /** Optional polling interval in milliseconds used when querying run status. */
    readonly pollInterval?: number;

    /** Optional upper bound for the polling interval backoff in milliseconds. */
    readonly maxPollInterval?: number;

    /** Optional request timeout in milliseconds for compiler service calls. */
...
export interface IWorkbenchConstructionOptions {
    ...
    /**
     * Optional configuration for the Universal Compiler service used by education features.
     */
