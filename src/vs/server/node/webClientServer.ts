...
export class WebClientServer {
    ...
    async handle(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery, pathname: string): Promise<void> {
        try {
            ...
            if (pathname === '/edu') {
                return this._handleRoot(req, res, parsedUrl, { templateResourcePath: 'vs/edu/browser/edu.workbench.html' });
            }
            if (pathname === '/') {
     ...
     */
    private async _handleRoot(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery, options?: { templateResourcePath?: string }): Promise<void> {
        ...
        const workbenchTemplateResource = options?.templateResourcePath ?? `vs/code/browser/workbench/workbench${this._environmentService.isBuilt ? '' : '-dev'}.html`;
        const filePath = FileAccess.asFileUri(workbenchTemplateResource).fsPath;
