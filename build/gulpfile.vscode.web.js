...
const eduWebResourceIncludes = [
    'out-build/vs/edu/browser/media/**/*.{css,svg,png}',
    'out-build/vs/edu/browser/edu.workbench.html'
];

const eduWebResources = [
    ...eduWebResourceIncludes,
    '!out-build/**/*.map'
];

const eduWebEntryPoints = [
    buildfile.workbenchEduWeb
];

const bundleEduWebTask = task.define('bundle-edu-web', task.series(
    util.rimraf('out-edu-web'),
    optimize.bundleTask({
        out: 'out-edu-web',
        esm: {
            src: 'out-build',
            entryPoints: eduWebEntryPoints,
            resources: eduWebResources
        }
    })
));

gulp.task(bundleEduWebTask);

const compileEduWebTask = task.define('compile-edu-web', task.series(
    compileBuildWithManglingTask,
    bundleEduWebTask
));

gulp.task(compileEduWebTask);

const rebuildEduWebTask = task.series(
    compileBuildWithoutManglingTask,
    bundleEduWebTask
);

const watchEduWebTask = task.define('watch-edu-web', () => {
    const run = () => rebuildEduWebTask();
    gulp.watch([
        'src/vs/edu/**/*.{ts,tsx,css,html}'
    ], run);
    return run();
});

gulp.task(watchEduWebTask);
